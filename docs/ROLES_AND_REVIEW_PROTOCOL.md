...
日期：2026-09-13。来源：用户本轮明确指定三种角色关系、Shanxi Docker CI 和合并前逻辑审查要求。本文规定后续协作方式，不追溯修改已关闭任务、冻结科学合同或 receipt。当前任务只从 [CURRENT_STATE](CURRENT_STATE.md) 的实时路由接手，旧总控保留历史来源。
...
| Sol | 经用户授权的理念/原则文档与适用 authority | architecture-aware TODO、科学具体化、验收设计、代码审核记录 | Astra | DS Flash 的代码、测试和 CI 证据是否满足 TODO |
...
- Sol 必须阅读并遵循 [`SOL_ROLE_PRINCIPLES.md`](SOL_ROLE_PRINCIPLES.md)。它规定 `Scientific Gap → Relevant Infra → Interface Map → Gap-to-Code → TODO → Adversarial Review` 的分阶段 authoring pipeline，并要求 TODO 明确可复用 infra、上下游 consumer、禁止重造路径、identity/RNG/provenance/formal gate。
- DS Flash / implementation agent 必须阅读并遵循 [`DS_FLASH_ROLE_PRINCIPLES.md`](DS_FLASH_ROLE_PRINCIPLES.md)。它规定 reuse-first、no semantic fallback、HIGH-risk counterexample、exact-SHA Shanxi CI、PR 多轮修复与 `READY_FOR_SOL_REVIEW` 终态。
- 旧 [`S_SERIES_TODO_AUTHORING_PRINCIPLES.md`](S_SERIES_TODO_AUTHORING_PRINCIPLES.md) 保留历史与兼容性；后续 Sol 角色入口以 `SOL_ROLE_PRINCIPLES.md` 为主。

这些模型名称表示用户指定的当前分工，不是模型能力证明，也不自动授予数据、运行或合并权限。替换执行模型不能取消对应的上游审查职责；执行者不能自己署名为 Sol/Astra 来补审核。

Astra 本角色不承担业务代码实现；Sol 对拆解、数学/判定具体化、architecture mapping 和代码正确性审查负责；DS Flash 在已授权范围内持续实现、修复并运行 CI。Sol 不得把代码审核退化为转述 DS 的报告，Astra 也不能仅凭 TODO 排版完整就认定任务可执行。

下游发现上游矛盾时，提交具体条款、最小反例和影响，交给相应上游修订。DS 不得为通过测试改目标或门槛；Sol 不得在 TODO 中偷换理念；Astra 改变用户取舍时须回到用户。已经明确且授权的其他工作继续，不要求用户逐个批准普通工程动作。

### 交付收尾与文档同步（2026-09-14）

Astra 除审理念，还审查要求是否具体可完成；Sol 对当前任务的实现、独立审核、集成与下一次运行准备负责，不能只交 TODO 后把上下文拼接留给用户。DS 自检仍不替代独立审核。

Sol 收尾同步 CURRENT_STATE 的执行基线、已解除/剩余阻塞和下一次运行条件；必要时更新 ARCHITECTURE_MAP 的实际组件对应。不为形式增加文件或审批层。新增要求说明影响范围，已接受工作只有受具体缺陷影响时才重审；现有必要 gate 不由本条豁免。
...
| Sol → Astra / DS | Scientific Gap、相关可复用 infra、上下游 interface map、沿用的 task ID、输入与实现范围、依赖、公式/控制/判据、禁止重造/semantic fallback、CI 入口和预算、交付与停止条件 |
| DS → Sol | 实际代码 SHA、净差异、完成的 task、复用的 existing surfaces、测试与运行来源、失败/未覆盖项、最早未完成项 |
| Sol → 合并执行者 | 被审查 SHA、相关文件/符号、formula/branch/identity/provenance 审查结果、Shanxi CI 链接与实际测试 SHA、未解决问题以及明确的代码接受/退回结论 |
...
当前仓库既有入口按各 stage TODO 和 live workflow 为准：

| 用途 | 规则 |
|---|---|
| 共享 CI 前置检查 | 优先复用 `.github/workflows/_cb16-shanxi-preflight.yml` 及当前 verified runner contract |
| 有界 authority / runtime smoke | 只覆盖其实际列出的测试，不能替新 stage 行为背书 |
| 已完成阶段资格入口 | 用于理解已验证接线；不要求因每个新 TODO 重跑已关闭阶段 |
| 新 stage 行为覆盖 | 由该 stage TODO 明确实现/绑定对应 workflow、专项测试与 artifact |
...
| identity / RNG / authority | behavior/target/eval、runtime/record/review/merge SHA、seed ownership、construction order | identity/RNG ledger；stale/mismatch counterexample；exact-SHA binding |
| provenance / proof | producer→durable fact→consumer→gate→artifact link 是否真的被验证 | 删除 scratch 后 artifact-only audit；破坏中间 link 必须 fail closed |
| gate 与最终 verdict | 比较方向、分母、全 seed 聚合、负控、缺失 gate、空结果、异常路径是否误判 PASS | 预构造 PASS/FAIL/边界结果；真实 manifest/audit mapping production-shape canary |

复杂表达式可以拆成有名中间量、明确辅助函数和决策表；不要求全项目改成某种语言，也不禁止必要分支。代码更短或“看起来优雅”不能成为接受理由。测试 oracle 不应复制同一套复杂公式，使实现和测试共享同一错误。

审查覆盖变更及直接受影响的调用关系，不无限重审整个历史仓库。发现具体错误，DS 修复并在 Shanxi CI 验证，Sol 复审；未解决的公式/控制流/identity/provenance 错误、缺少必要 CI 或缺少 Sol 审核，均不能将该代码合并 main。有效 `SCIENTIFIC_FAIL` 则按原任务证据与收尾规则处理，不能为了让科学结果变绿而改目标。
