...
def test_w02_ordered_legs_hashes_and_boundary_taxonomy():
    ...
    for bad in (replace(t,execution_legs=(replace(leg,leg_index=1),)),replace(t,post_account_truth_hash='x'),replace(t,boundary_type='UNKNOWN_BOUNDARY')):
        with pytest.raises(RuntimeError): bad.validate()
