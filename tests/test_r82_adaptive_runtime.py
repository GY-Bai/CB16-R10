...
def test_r82_runtime_overlay_starts_six_workers_and_preserves_cache_identity():
    rp = load_r102_runtime_parallelism(Path(__file__).resolve().parents[1], live_environment_check=False)
    assert rp.performance_overlay == "R8_2_6W_RAM_ADAPTIVE"
    assert rp.h72_workers == 6
    assert rp.h72_threads_per_worker == 1
    assert rp.h72_max_in_flight >= 6
...
def test_ram_monitor_retires_then_kills_one_worker_at_thresholds():
    assert ram_action_r82(0.50, 6, high=0.85, hard=0.92) == "NONE"
    assert ram_action_r82(0.85, 6, high=0.85, hard=0.92) == "RETIRE_ONE"
