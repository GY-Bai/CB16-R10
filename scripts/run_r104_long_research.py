...
from cb16_local_opt.r102_evidence_incremental import prepare_r104_incremental_cache
...
def main() -> int:
    ...
    with RunRootExclusiveLock(Path(a.run_root)):
        # On a clean R10.4 root, densify 512h -> 256h incrementally: verify and
        # reuse the R10.3 anchor subset, then compute only newly introduced anchors.
        # If R10.4 already has its canonical cache this is an O(1) manifest reuse.
        prepare_r104_incremental_cache(
            package_root=a.package_root,
            data_root=a.data_root,
            r103_root=a.r103_root,
            r104_run_root=a.run_root,
            device=a.device,
