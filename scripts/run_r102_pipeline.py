...
DEFAULT_RUN_ROOT='/home/bgy/cb16_ssd/runtime/R10_2'
...
def _write_failure_receipt(run_root, exc):
    ...
    out={"schema":"CB16_R10_2_1_EXECUTION_BLOCKER_RECEIPT_V1","status":"ENGINEERING_EXECUTION_BLOCKED","phase":"R10_2","exception_type":type(exc).__name__,"exception":str(exc),"traceback":traceback.format_exc(),"scientific_verdict_changed":False,"final_holdout_2025_09_accessed":False,"instruction":"Do not convert this exception into scientific FAIL; repair only the engineering compatibility cause and rerun from the same frozen authority."}
...
def _safe_blocker_summary(exc):
    ...
    if isinstance(exc, ModuleNotFoundError): code='MISSING_PYTHON_MODULE'; detail=getattr(exc,'name',None)
    elif isinstance(exc, PermissionError):
        code='HOST_PATH_PERMISSION'; fn=getattr(exc,'filename',None); detail=Path(fn).name if fn else None
    elif isinstance(exc, FileNotFoundError):
        code='HOST_FILE_OR_ASSET_MISSING'; fn=getattr(exc,'filename',None); detail=Path(fn).name if fn else None
...
def _append_ci_blocker_summary(exc):
    ...
    if not ci_out: return
    summary=_safe_blocker_summary(exc)
    with (Path(ci_out)/'REPORT.md').open('a',encoding='utf-8') as f:
...
def main() -> int:
    ...
    ap.add_argument('--run-root', default=host_binding('CB16_R10_R102_ROOT', DEFAULT_RUN_ROOT))
    ...
    args=ap.parse_args()
    result=run_campaign(package_root=args.package_root,data_root=args.data_root,run_root=args.run_root,parent_r101_root=args.parent_r101_root,parent_g0=args.parent_g0,device=args.device,attempts=5,stride_hours=512,prehistory_hours=96,epochs=12,batch_size=512,lr=3e-4,verify_checksum_samples=True,verify_all_cache_checksums=args.verify_all_cache_checksums,profile_name='R10_2_5GEN_QUALIFICATION')
    append_scientific_summary(result,'R10_2')
    ...
    return 0 if status_is_pass(result,'R10_2') else 2
...
if __name__=='__main__':
    try:
        ...
        try: _append_ci_blocker_summary(exc)
        except Exception: pass
        try: _write_failure_receipt(host_binding('CB16_R10_R102_ROOT',DEFAULT_RUN_ROOT),exc)
        except Exception: pass
        raise
    else:
        raise SystemExit(rc)
