...
DEFAULT_R102_ROOT='/home/bgy/cb16_ssd/runtime/R10_2'
DEFAULT_RUN_ROOT='/home/bgy/cb16_ssd/runtime/R10_3'
...
def _local_receipt(run_root,exc):
    ...
    (rr/'EXECUTION_BLOCKER_RECEIPT_R103.json').write_text(json.dumps({"schema":"CB16_R10_ENGINEERING_BLOCKER_V1","phase":"R10_3","exception_type":type(exc).__name__,"exception":str(exc),"traceback":traceback.format_exc(),"scientific_verdict_changed":False,"final_holdout_2025_09_accessed":False},indent=2,sort_keys=True)+'\n')
...
def main() -> int:
    ...
    ap.add_argument('--r102-root',default=host_binding('CB16_R10_R102_ROOT',DEFAULT_R102_ROOT))
    ap.add_argument('--run-root',default=host_binding('CB16_R10_R103_ROOT',DEFAULT_RUN_ROOT))
    ...
    r=run_campaign(package_root=a.package_root,data_root=a.data_root,run_root=a.run_root,parent_r101_root=a.parent_r101_root,parent_g0=a.parent_g0,device=a.device,symbols=ALL_SUPPORTED_SYMBOLS_R102,attempts=20,stride_hours=512,prehistory_hours=96,epochs=12,batch_size=512,lr=3e-4,profile_name='R10_3_10ASSET_20GEN_EXPANSION',prerequisite_result=prev,start_checkpoint=start)
    append_scientific_summary(r,'R10_3')
    print(json.dumps({'final_status':r.get('final_status'),'return_bundle':r.get('return_bundle')},indent=2))
    return 0 if status_is_pass(r,'R10_3') else 2
...
if __name__=='__main__':
    try: rc=main()
        ...
        try: _local_receipt(host_binding('CB16_R10_R103_ROOT',DEFAULT_RUN_ROOT),exc)
        except Exception: pass
        raise
