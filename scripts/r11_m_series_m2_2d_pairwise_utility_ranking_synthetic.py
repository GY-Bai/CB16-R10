...
SCHEMA="CB16_R11_M_SERIES_M2_2D_PAIRWISE_UTILITY_RANKING_SYNTHETIC_RESULT_V1"
PREREG_COMMIT="32632987b176d11f6afb055375a4b2b96c99599d"
GATE_BLOB="5ded467bf80ad7071a57cddd37315599fd7ce701"
M22C_ADJ_BLOB="fe9dc161da3bb549af8360b22bda95030b75f5f1"
...
def verify_repo()->dict[str,Any]:
    require(subprocess.call(["git","merge-base","--is-ancestor",PREREG_COMMIT,"HEAD"])==0,"M22D_NOT_DESCENDED_FROM_PREREGISTRATION")
    exact={
        "gate":("authority/rearchitecture_r11/CB16_R11_M_SERIES_M2_2D_PAIRWISE_UTILITY_RANKING_SYNTHETIC_GATE_V1.json",GATE_BLOB),
        "m22c_adjudication":("authority/rearchitecture_r11/CB16_R11_M_SERIES_M2_2C_BALANCED_PRESERVE_ADV_CORRECTION_SYNTHETIC_ADJUDICATION_V1.json",M22C_ADJ_BLOB),
    }
    observed={}
    for name,(path,expected) in exact.items():
        got=git("rev-parse",f"HEAD:{path}")
        require(got==expected,f"M22D_IMMUTABLE_BLOB_DRIFT:{name}:{got}")
        observed[name]=got
...
def runtime_identity(device:str)->dict[str,Any]:
    ...
    if dev.type=="cuda":
        require(torch.cuda.is_available(),"M22D_CUDA_REQUIRED_BUT_UNAVAILABLE")
...
def main()->int:
    ...
    for regime in REGIMES_M22D:
        for seed in regime.seeds:
            cells.append(run_seed_cell_m22d(seed=seed,regime=regime,device=args.device))
    summary=adjudicate_m22d(cells)
    result={
        "schema":SCHEMA,
        "status":"M2_2D_PAIRWISE_UTILITY_RANKING_SYNTHETIC_COMPLETE",
        "repo":repo,
        ...
        "protocol":{
            ...
            "arms_per_cell":24,
            "total_arms":480,
            "candidate_probability_target":False,
            "candidate_all_three_direction_pairs":True,
            "candidate_temperature_threshold_or_margin_hyperparameter":False,
        ...
        "next_legal_step":(
            "ADJUDICATE_M2_2D_THEN_PREREGISTER_CONSUMED_R6_TRAIN_ONLY_PAIRWISE_DIRECTION_SHADOW"
            if summary["overall_pass"] else
            "ADJUDICATE_M2_2D_FAILURE_AND_TEST_DIRECT_TEACHER_DECISION_REGRET_SURROGATE_IN_SYNTHETIC_SPACE"
        ),
    }
