...
It receives before/after Student outputs plus the existing reduced Teacher Direction
probabilities and attributes discrete Direction migration relative to the immutable G0.
"""
...
TEACHER_DIRECTION_SOFTMAX_TEMPERATURE = 0.002
...
def direction_advantage_from_teacher_probs(
    teacher_probs: Any,
    g0_direction: Any,
    *,
    temperature: float = TEACHER_DIRECTION_SOFTMAX_TEMPERATURE,
) -> np.ndarray:
    """Recover best-Direction mean-utility advantage over the G0 Direction.

    Existing Teacher Direction probabilities are softmax(mu_d / tau), where mu_d is
    the best mean utility within Direction d. Therefore
      tau * [log p(best) - log p(g0)] == mu_best - mu_g0.
    No continuous requested-risk projection and no quantile subtraction are involved.
    """
    p = _prob_matrix(teacher_probs, "TEACHER_PROBS")
    g = np.asarray(g0_direction, dtype=np.int64).reshape(-1)
    require(len(g) == len(p), "M0_G0_DIRECTION_LENGTH")
    require(np.all((g >= 0) & (g <= 2)), "M0_G0_DIRECTION_RANGE")
    require(float(temperature) > 0.0, "M0_BAD_TEMPERATURE")
    best = np.argmax(p, axis=1)
    row = np.arange(len(p))
    tiny = np.finfo(np.float64).tiny
    advantage = float(temperature) * (
        np.log(np.clip(p[row, best], tiny, 1.0))
        - np.log(np.clip(p[row, g], tiny, 1.0))
    )
    advantage[np.abs(advantage) < 1e-15] = 0.0
    require(np.all(advantage >= -1e-12), "M0_NEGATIVE_BEST_DIRECTION_ADVANTAGE")
    return np.maximum(advantage, 0.0)
...
def attribute_direction_migration_m0(
    *,
    ...
    advantage = direction_advantage_from_teacher_probs(teacher, g0_dir)
            ...
            quartiles.append(
                {
                    ...
                    if not np.any(mask)
                    else float(np.mean(direct_d[mask])),
    ...
    summary = {
        ...
        "direct_teacher_correction_fraction_of_all_direction_changes": _safe_rate(
            direct_count, change_count
        ),
        "non_teacher_attributable_fraction_of_all_direction_changes": _safe_rate(
            non_teacher_count, change_count
        ),
        "teacher_g0_agreement_rows": agreement_count,
        "teacher_g0_disagreement_rows": disagreement_count,
        "teacher_g0_agreement_direction_change_rate": _safe_rate(
            agreement_change_count, agreement_count
        ),
        "teacher_g0_disagreement_move_to_teacher_rate": _safe_rate(
            direct_count, disagreement_count
        ),
        ...
        per_fold.append(
            {
                ...
                "teacher_g0_agreement_direction_change_rate": x[
                    "teacher_g0_agreement_direction_change_rate"
                ],
                "teacher_g0_disagreement_move_to_teacher_rate": x[
                    "teacher_g0_disagreement_move_to_teacher_rate"
                ],
            }
        )
